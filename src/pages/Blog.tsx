  ...
  useEffect(() => {
    const fetchArticles = async () => {
      const {
        data,
        error
      } = await supabase.from("articles").select("*").order("publish_date", {
        ascending: false
      });
      if (error) {
      ...
      <main className="pt-16">
        ...
        <section className="bg-gradient-to-br from-primary to-accent py-20 px-4">
          <div className="container mx-auto max-w-4xl text-center">
            <h1 className="text-4xl md:text-5xl font-heading font-bold text-primary-foreground mb-4">Explore Destinos com a Travel</h1>
          ...
          <div className="container mx-auto max-w-6xl">
            {loading ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                {articles.map(article => <Card key={article.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    <img src={article.hero_image} alt={article.title} className="w-full h-48 object-cover" />
                    <CardContent className="p-6">
